...
public class GeneratorPlikuParametrycznegoGry {
     ...
     */
    public GeneratorPlikuParametrycznegoGry() {
        do {
            p = sumaCyfr(p);
        } while (p > 9);
    }
    ...
    public void zapiszPlikParametryczny(String nazwaPliku) {
        PrintWriter pw = null;
        try {
            pw = new PrintWriter(new BufferedWriter(new FileWriter(nazwaPliku)));
        } catch (IOException ioe) {
            pin.wypiszKomunikatyIZakoncz("Nie udało się otworzyć pliku parametrycznego " +
        ...
        if (losowo(0, 1) == 0) { // obiekty rysowane jako kształty
            ...
            pw.println("obiektyGry=plikGraficzny" + lsep
                    + "plikObiektu=plikObiektu.jpg" + lsep
